...
const NearbyClinicsMap = ({ pharmacies }) => {
                        ...
                        <div className="display-map">
                            <MapContainer center={mapCenter} zoom={ZOOM_LEVEL} className="MapContainer" ref={mapRef} onClick={handleMapClick}>
                                ...
                                {pharmacies.map((pharmacy, idx) => {
                                    const [clinicLat, clinicLong] = pharmacy.pharmacy_location ? pharmacy.pharmacy_location.split(",").map(parseFloat) : [0, 0];
                                    console.log(clinicLat, clinicLong)
                                    console.log('Doctor clinics are: ', pharmacy)
                                    return(
                                    <Marker position={[clinicLat, clinicLong]} icon={markerIcon} key={idx}>
                                        <Popup>
                                            <div className="popup-div">
                                                <div className="top">
                                                    <div className={`image ${isWithinTimeRange(pharmacy.opening_time, pharmacy.closing_time)?'online':'offline'}`} style={pharmacy ? {
